...
- **Mission altitude modes** - Mission planner now switches between relative-to-takeoff and absolute (WGS84) height modes; absolute mode samples terrain DEM (Terrarium) via the new cache to seed waypoint altitudes above ground automatically.
- **Map controls** - Auto-center/auto-rotate respect manual panning, follow the aircraft by default (and fall back to home/manual targets only when aircraft telemetry is absent), terrain preview toggles preserve zoom/bearing/pitch, layer presets add street/satellite/hybrid options, and terrain caching/exaggeration live in Advanced while the terrain toggle sits beside the 2D/3D switch. Click (WP) / Ctrl⌘+click (stage) / Alt+click (POI) combos no longer interfere with drag-to-pan.
